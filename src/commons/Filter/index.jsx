...
import { setNftData } from "../../redux/slice/Nft";
...
const Filter = ({
  ...
  const handleFilter = async () => {
    ...
    try {
      const response = await handleApiCall(
        "post",
        `${endpoints.getNftsList}`,
        data
      );
      if (response?.data?.success) {
        dispatch(setNftData(response?.data?.data));
      } else {
        dispatch(setNftData([]));
      }
    } catch (error) {
        ...
        <div className="panel panel-default">
          ...
          <div id="collapse2" className="panel-collapse collapse in show">
            <div className="panel-body price-range-wrap">
              <select name="dollor" className="united-states-dollor" disabled>
