(function($){
	...
	that_nodes.find("ul").hide();
	that_nodes.find('.' + expandedSymbol).removeClass(expandedSymbol).addClass(collapsedSymbol);
    };

    //------------------expandAll------------------//
    $.fn.hummingbird.expandAll = function(tree,collapsedSymbol,expandedSymbol){
	var that_nodes = tree.find('label:not(.disableToggle)');
	that_nodes.find("ul").show();
	that_nodes.find('.' + collapsedSymbol).removeClass(collapsedSymbol).addClass(expandedSymbol);
    };

    //-------------------collapseSingle---------------//
    $.fn.hummingbird.collapseSingle = function(node,collapsedSymbol,expandedSymbol){
	if (!node.next('label').hasClass('disableToggle')){
	    node.parent().next("ul").hide();
	...
	if (!that_label.hasClass('disableToggle')){
	    var that_ul = that_label.find("ul");
	    that_ul.show().find("i").removeClass(collapsedSymbol).addClass(expandedSymbol);
	    //expand all parents and change symbol
	    if (expandParents === true) {
		that_node.parents("ul").show().find("i").removeClass(collapsedSymbol).addClass(expandedSymbol);
	    }
	}
    };
	...
	if (!that_label.hasClass('disableToggle')){
	    ...
	    if (collapseChildren === true) {
		that_node.parent("label").parent("li").find("ul").hide().find("i").removeClass(expandedSymbol).addClass(collapsedSymbol);
	    } else {	    
		that_ul.hide().find("i").removeClass(expandedSymbol).addClass(collapsedSymbol);
	    }
	}
    };
	...
	if (attr == "text") {
	    ...
	    that_nodes.find('input:checkbox').trigger("click");
	} else {
